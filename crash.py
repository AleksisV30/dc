...
from config import (
    MIN_BET, MAX_BET, BETTING_SECONDS, HOUSE_EDGE_CRASH, D, q2, now_utc, iso
)
from shared import with_conn, get_balance
...
def begin_running(cur, round_id: int):
    ...
    now = now_utc(); exp_end = now + datetime.timedelta(seconds=dur)
...
def resolve_round_end(cur, round_id: int, bust: float):
    ...
    for uid, bet, goal, cashed, resolved, win in bets:
        uid = str(uid); bet=Decimal(bet); goal=float(goal); win=Decimal(win); resolved=bool(resolved)
        ...
        if not resolved:
            if goal <= bust:
                ...
                cur.execute("""UPDATE crash_bets SET resolved=TRUE WHERE round_id=%s AND user_id=%s""",
                            (round_id, uid))
                win = Decimal("0.00"); cashed_val = goal
            xp_gain = max(1, min(int(bet), 50))
            cur.execute("""INSERT INTO crash_games(user_id,bet,cashout,bust,win,xp_gain)
                           VALUES(%s,%s,%s,%s,%s,%s)""",
...
def finish_round(cur, round_id: int):
    ...
    resolve_round_end(round_id, bust)  # reuse decorator
    cur.execute("""UPDATE crash_rounds
...
def your_bet(cur, round_id: int, user_id: str):
    ...
    r = cur.fetchone()
    if not r: return None
    return {"bet": float(q2(Decimal(r[0]))), "cashout": float(r[1]),
            "cashed_out": (float(r[2]) if r[2] is not None else None),
            "resolved": bool(r[3]), "win": float(q2(Decimal(r[4])))}
...
def your_history(cur, user_id: str, limit: int = 10):
    ...
    return [{"bet":float(q2(Decimal(r[0]))),"cashout":float(r[1]),"bust":float(r[2]),"win":float(q2(Decimal(r[3]))),"xp_gain":int(r[4]),"created_at":str(r[5])} for r in cur.fetchall()]
...
@router.post("/place")
def api_crash_place(user_id: str, body: CrashBetIn):
    bet = q2(Decimal(body.bet or "0"))
    cashout = float(body.cashout or 2.0)
    return place_bet(user_id, bet, max(1.01, cashout))
...
def cashout_now(cur, user_id: str) -> Dict:
    ...
    bet, cash_goal, cashed_out, resolved = Decimal(b[0]), float(b[1]), b[2], bool(b[3])
...
@router.post("/cashout")
def api_crash_cashout(user_id: str):
    return cashout_now(user_id)

@router.get("/history")
def api_crash_history(user_id: str):
    return {"rows": your_history(user_id, 10)}
